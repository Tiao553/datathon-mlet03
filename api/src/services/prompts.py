...
def prompt_vaga(data):
    return f"""
Você é um especialista em Recursos Humanos com foco em análise de descrições de vagas.
Sua tarefa é extrair e estruturar todas as informações relevantes se existir.

Caso algum dado não esteja presente, use “Não mencionado”.
Título: titudo da vaga que foi disponibilizada pela empresa
Atividades: atividades desejadas pela empresa
Competências: todas as competencias que a empresa deseja para a vaga
Observações: todas as demais observações que a empresa adicionou sobre a vaga
Habilidades: comportamentais: habilidades comportamentais que a empresa deseja para a vaga
...
abaixo esta o json que sera usado para o preenchimento:
{data}
""".strip()


def prompt_candidato(data):
    return f"""
...
def chamar_deepseek(prompt: str) -> str:
    ...
    """
    client = OpenAI(api_key=os.getenv("DEEPSEEK-KEY"), base_url="https://api.deepseek.com/v1")
    try:
