...
import numpy as np   # Para operações numéricas, muito usado por outras bibliotecas.
from sklearn.model_selection import train_test_split, RandomizedSearchCV # Para dividir os dados e otimizar o modelo.
from sklearn.metrics import roc_auc_score, classification_report, ConfusionMatrixDisplay # Para avaliar a performance do modelo.
from sklearn.compose import ColumnTransformer # Para aplicar diferentes preparações a diferentes colunas.
from sklearn.preprocessing import OneHotEncoder, StandardScaler # Para preparar os dados para o modelo (normalizar números e codificar categorias).
...
import lightgbm as lgb # O nosso modelo avançado de Machine Learning (LightGBM).
from scipy.stats import randint, uniform # Para definir as faixas de busca aleatória dos hiperparâmetros.
...
# Se o Python não encontrar o arquivo no caminho especificado (gerando um `FileNotFoundError`), ele executa o bloco `except`.
    ...
    print(f"ERRO: O arquivo não foi encontrado em '{caminho_base}'. Verifique o caminho.")
...
common_recruiters = recruiter_counts.filter(pl.col("count") >= min_frequency)["p_recrutador"]
...
df = df.with_columns(
    ...
    percentual_perfil_completo=(pl.sum_horizontal(pl.col(c).is_not_null() for c in colunas_perfil_app) / len(colunas_perfil_app)).fill_null(0),
    ...
    tem_objetivo_profissional=(pl.col('app_ib_objetivo_profissional').is_not_null() & (pl.col('app_ib_objetivo_profissional') != "")).cast(pl.Int8),
    tem_remuneracao_definida=(pl.col('app_ip_remuneracao').is_not_null() & (pl.col('app_ip_remuneracao') != "")).cast(pl.Int8),
    ...
    contem_palavra_chave_negativa=pl.col('p_comentario').str.contains(regex_palavras_negativas).fill_null(False).cast(pl.Int8),
    ...
    dias_para_se_candidatar=(pl.col('p_data_candidatura').dt.epoch(time_unit="d") - pl.col('job_ib_data_requicisao').dt.epoch(time_unit="d")).fill_null(-1),
    # Usa a flag que já existia na base, garantindo que valores nulos se tornem 0.
    candidatura_proativa=pl.col('candidatura_antes_da_abertura_oficial').fill_null(0).cast(pl.Int8),
    ...
    dias_desde_ultima_atualizacao=(pl.lit(datetime.datetime.now(datetime.timezone.utc)).dt.epoch(time_unit="d") - pl.col('p_ultima_atualizacao').dt.epoch(time_unit="d")).fill_null(-1),
    # Usa o regex positivo para verificar se o comentário contém palavras-chave de interesse.
    contem_palavra_chave_positiva=pl.col('p_comentario').str.contains(regex_palavras_positivas).fill_null(False).cast(pl.Int8),
    # Calcula a duração do processo para um candidato (da candidatura até a última atualização).
    dias_no_processo=(pl.col('p_ultima_atualizacao').dt.epoch(time_unit="d") - pl.col('p_data_candidatura').dt.epoch(time_unit="d")).fill_null(-1),
    ...
    p_recrutador_tratado = pl.when(pl.col("p_recrutador").is_in(common_recruiters)).then(pl.col("p_recrutador")).otherwise(pl.lit("Outros"))
)
...
df = df.with_columns(
    sentimento_comentario_score = pl.when(pl.col('contem_palavra_chave_positiva') == 1).then(1).when(pl.col('contem_palavra_chave_negativa') == 1).then(-1).otherwise(0)
)
...
features_categoricas = ['p_recrutador_tratado'] # Colunas de texto/categoria que serão codificadas.
features_finais_df = features_para_escalar + features_categoricas # Lista com todas as features.
...
param_distributions = {
    ...
    'colsample_bytree': uniform(0.7, 0.3)    # Fração de features a usar por árvore
}
...
random_search = RandomizedSearchCV(
    ...
    n_jobs=1,                                  # Execução sequencial para evitar problemas.
...
print("\nBusca concluída!")
print(f"Melhor Score AUC (em validação cruzada): {random_search.best_score_:.4f}")
...
try:
    ...
    ConfusionMatrixDisplay.from_estimator(best_model, X_test_processed, y_test, ax=ax, cmap='Blues')
